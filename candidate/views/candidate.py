...
from candidate.dto.register import RegisterCandidateRequest, InviteCandidateRequest
...
class InviteCandidateView(GenericAPIView):
    def post(self, request):
        data = request.data
        request_data = convert_json_to_pydantic(data, InviteCandidateRequest)
        new_candidate =  register_candidate(request_data)
        res = model_to_json(new_candidate)
        return Response(status=200, data=res)
